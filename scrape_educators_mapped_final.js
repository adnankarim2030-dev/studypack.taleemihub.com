...
(async () => {
      ...
      try {
        ...
        if (json.data && Array.isArray(json.data)) {
            ...
            for (const p of json.data) {
                let title = p.title || p.productName || p.name;
                if (title && p.id) {
                    if (!productsMap[p.id]) {
                        productsMap[p.id] = {
                            title: title,
                            price: p.hasVariants && p.variants ? p.variants[0].price : p.price || 0,
                            img: (p.productImage && p.productImage.length > 0) ? 'https://cdn.fishry.com/product/' + p.productImage[0].Image : (p.image || (p.images && p.images.length > 0 ? (p.images[0].src || p.images[0].url) : '')),
  ...
  fs.writeFileSync('educators_products_mapped_final.json', JSON.stringify(finalProducts, null, 2));
  await browser.close();
